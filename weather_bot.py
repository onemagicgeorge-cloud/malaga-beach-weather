...
def generate_fallback_message(d):
    ...
    trend_text = ""
    if len(hourly) >= 3:
        temps_list = [h["temp"] for h in hourly if h["temp"] is not None]
        codes_list = [h["code"] for h in hourly if h["code"] is not None]
        if temps_list:
            if max(temps_list) - min(temps_list) <= 3:
                temp_trend = "температура стабільна"
            elif temps_list[-1] > temps_list[0]:
                temp_trend = "потеплішає"
            else:
                temp_trend = "похолоднішає"
            if codes_list:
                sunny = sum(1 for c in codes_list if c in [0, 1])
                cloudy = sum(1 for c in codes_list if c in [2, 3])
                rainy = sum(1 for c in codes_list if c in [51, 61, 95])
                if rainy > len(codes_list)//2:
...
def generate_ai_message(d):
    ...
    if tod == "morning":
        prompt = (
            ...
            f"українською (до 500 символів) для пляжу {BEACH_NAME}.\n"
        ...
        prompt += "Погодинний прогноз на сьогодні (температура, вітер, UV):\n"
        for h in d.get("hourly_forecast", [])[::3]:
            prompt += f"  {h['hour']}:00 — {h['temp']}°C, вітер {h.get('wind','?')} км/год, UV {h.get('uv','?')}, код {h['code']}\n"
        prompt += (
        ...
        prompt = (
            ...
            f"українською (до 450 символів) для пляжу {BEACH_NAME}.\n"
            f"Зараз: {c['temperature']}°C, вітер {c['windspeed']} км/год, "
            f"код погоди {c['weathercode']}\n"
            f"{water_info}"
        )
        ...
        prompt += "Прогноз на другу половину дня (температура, вітер, UV):\n"
